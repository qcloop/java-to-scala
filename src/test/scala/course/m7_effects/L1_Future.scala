...
object L1_Future extends Lesson {
  ...
  val testSuccessful = suite("successful")(
    testM("successful") {
      lazy val future: Future[String] = Future.successful("Howdy!")
  ...
  val testFailed = suite("failed")(
    testM("failed") {
      val future: Future[Int] = Future.failed(BadTime("Oh no!"))

      testFuture(future.recover(t => t)) { result =>
        assertTrue(result.right.get.asInstanceOf[BadTime] == BadTime("Oh no!"))
      }
    }
  )
  /** ✏ EXERCISE
  ...
  val testForComprehension = suite("for comprehension")(
    testM("for comprehension") {
      ...
      val future: Future[Int] = for {
        ...
        c <- asyncLineCount(text)
        _ <- saveToDatabase(text)
      } yield c
